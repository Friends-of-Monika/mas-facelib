...
// apiResponse is the JSON envelope every analyze call returns.
...
// fl_analyze detects faces in a raw pixel buffer and classifies their emotion.
//
...
// so the caller may free it as soon as fl_analyze returns. The returned string
...
func fl_analyze(
	...
	if opts_json != nil {
		if raw := C.GoString(opts_json); raw != "" {
			var o options
